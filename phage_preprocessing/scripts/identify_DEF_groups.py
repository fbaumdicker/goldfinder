...
def search_and_extract(csv_file, search_word):
    with open("/home/franz/syncedprojects/RoyalSocietyManuskriptData/gene_presence_absence_roary.csv", newline='', encoding='utf-8') as f:
        reader = csv.reader(f)
        for row in reader:
...
file_path = "allbacterialdefensegenes.csv"
...
# Save to CSV
with open("bacterial_defensegroups.csv", "w", newline="") as f:
    writer = csv.writer(f)
